...
const GetAllEmployeeSalary = () => {
  ...
  useEffect(() => {
    async function gedData() {
      try {
        const response = await axios.get(`${APIURL}/emp_sal/ge_all_salary`);
        if (response.status === 200) {
          setEmpSal(response.data.EmployeeList);
  ...
  const downloadReport = () => {
    doc.text("Employee Salary Report", 30, 10);

    let array = [];
    empSal.map((empSals, index) => {
      let row = [];
      row.push(index + 1);
      row.push(empSals.employee_id);
      row.push(empSals.employee_name);
      row.push(empSals.employee_post);
      row.push(empSals.employee_basicSalary);
      row.push(empSals.employee_start_date);
      row.push(empSals.employee_end_date);
      row.push(empSals.employee_working_days);
      row.push(empSals.employee_ot);
      row.push(empSals.employee_epf);
      row.push(empSals.employee_etf);
      row.push(empSals.employee_totSal);
      array.push(row);
      return row;
    });
      ...
      .then((res) => {
        ...
        if (res.data.code === 200) {
          ...
          window.setTimeout(function () {
            window.location.href = "/get_all_salary";
          }, 1000);
        } else {
  ...
  return (
    <div>
                              ...
                              {item.isPay === 2 && (
                                <>
                                  ...
                                  <td>
                                    ...
                                    >
                                      <p
                                        className="p-1 mb-1 bg-warning text-white"
                                        style={{
                                          alignContent: "center",
                                          textAlign: "center",
                                        }}
      ...
      {isLoading ? (
        ...
        <div className="container text-center py-5">
          <h3 style={{ marginTop: 50 }}>No Medicine found</h3>
        </div>
      )}
    </div>
  );
...
// }
export default GetAllEmployeeSalary;
